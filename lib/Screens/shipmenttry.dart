...
class ShipmentsTry extends StatefulWidget {
  static String id = 'shipments';
  @override
  _ShipmentsTryState createState() => _ShipmentsTryState();
}

class _ShipmentsTryState extends State<ShipmentsTry> {
  ...
  String provider_category_details,
      ...
      fromDate,
      toDate;
  int container_id_details, fuel_type_id_details, provider_id_details;

  int add_Volume;
  Timestamp myTimeStamp, myTimeStamp1;
  int container_id, fuel_type_id, provider_id;
  ...
  Widget build(BuildContext context) {
    ...
    // print('ppppppppppppppppppppppptttttttt $totalVolume $totalDebt $totalPaid');
    getTotalVolume();
    getDebtMoney();
    getPaidMoney();
    getLastDocId();
                                    ...
                                    child: StreamBuilder(
                                      ...
                                      builder: (context, snapshot) {
                                        ...
                                        if (!snapshot.hasData) {
                                          ...
                                          print(
                                              'fffffffffffffffffffff $totalDebt $totalPaid');
                                          return datatable(
                                              list: snapshot.data.docs,
                                              notifyParent: refresh);
                                        }
...
class datatable extends StatefulWidget {
  ...
  const datatable({this.list, this.notifyParent});

  @override
  _datatableState createState() =>
      _datatableState(list: this.list, notifyParent: this.notifyParent);
}
...
class _datatableState extends State<datatable> {
  ...
  _datatableState({this.list, @required this.notifyParent});
          ...
          child: DataTable(
            ...
            rows: list.map((shipment) {
              ...
              return DataRow(cells: [
                DataCell(
                  Text(
                      '${formatter.format(DateTime.tryParse((shipment["Shipment_Date"]).toDate().toString())).toString()}'),
                ),
                DataCell(
                  Text('${shipment["Shipment_Volume"]}'),
                ),
                DataCell(
                  Text('Container ${shipment["Container_Name"]}'),
                ),
                DataCell(
                  Text('Provider ${shipment["Provider_Name"]}'),
                ),
                DataCell(
                  Text('${shipment["Shipment_Value"]}'),
                ),
                DataCell(
                        ...
                        GestureDetector(
                          ...
                          onPressed: () {
                            db
                                ...
                                .doc(station)
                                .collection("Shipment")
                                .doc((shipment["Shipment_Id"]).toString())
                                .update({
                              "IsPaid": true,
                              "Paid_Date": Timestamp.fromDate(today)
                            }).then((value) {
                              ...
                              print("error update payyyyyyyy $onError");
                            });
                          },
                          child: Text('Pay'),
                          style: ButtonStyle(
                ...
                DataCell(
                  GestureDetector(
                    ...
                    color: Colors.red,
                    onPressed: () {
                      var previous_shipment = widget.list[1];
                      shipmentToDelete = previous_shipment['Shipment_Volume'];

                      shipmentToDelete = shipment['Shipment_Volume'];
                      db
                          .collection('Stations')
                          .doc(station)
                          .collection('Container')
                                ...
                                if (val.docs.length > 0)
                                  {
                                    ...
                                    print('volumeeeeeeeeee $currentContVolume'),
                                    volumeToBeAdded =
                                        currentContVolume - shipmentToDelete,
                                  }
                                else
                                  {
                                    print("elseeeee"),
                      ...
                      print("Deleteeeeeeeeeeeeeeeee");
                      int docID = shipment['Shipment_Id'];
                      String docId = docID.toString();
                      showDialog<String>(
                        context: context,
                                            ...
                                            if (val.docs.length > 0)
                                              {
                                                ...
                                                //for (var i = 0; i < val.docs.length; i++)
                                                //{
                                                containerrr_iddd = val.docs[0]
                                                    .get("Container_Id"),
                                                oldddd_volumeee =
                                                    val.docs[0].get("Volume"),

                                                print(
                                                    'conttttttt idddddddddddd $containerrr_iddd'),
                                                print(
                                                    'olddddd volumeeeee $oldddd_volumeee'),
                                                //},

                                                add_Volume = (oldddd_volumeee -
                                                    shipment[
                                                        'Shipment_Volume']), //documentSnapshot['Shipment_Volume'];
                                                print(
                                                    'adddddddddd volumeeeeeeeee $add_Volume'),
                                                print(
                                                    'shipmentttt volumeeeeee ${shipment['Shipment_Volume']}'),
                                                db
                                                    .collection('Stations')
                                                    .doc(station)
                                                    .collection('Container')
                                                    .doc(containerrr_iddd
                                                        .toString())
                                                    .update({
                                                  'Volume': add_Volume
                                                }).then((value) {
